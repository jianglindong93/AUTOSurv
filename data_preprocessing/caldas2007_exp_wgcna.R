...
setwd("D:/AUTOSurv/ICGC_data/Caldas2007/divided_data_new/div_5/train_test_split/no_normalization")
# genes
data_gene<-read.csv("data_train_overall.csv", h = T)
datExpr<-data_gene[, -c(1:9)]
...
setwd("D:/AUTOSurv/ICGC_data/Caldas2007/divided_data_new/div_5/WGCNA")
net = blockwiseModules(datExpr, power = 3, maxBlockSize = 4189,
                       TOMType = "unsigned", minModuleSize = 30,
                       reassignThreshold = 0, mergeCutHeight = 0.25,
                       numericLabels = TRUE, pamRespectsDendro = FALSE,
                       saveTOMs = TRUE,
                       saveTOMFileBase = "Caldas2007_gene",
                       verbose = 3)
table(net$colors)
...
save(MEs, moduleLabels, moduleColors, geneTree,
     file = "caldas2007_gene-05-networkConstruction-auto.RData")
rownames(MEs)<-c(1:nrow(MEs))

setwd("D:/AUTOSurv/ICGC_data/Caldas2007/divided_data_new/div_5/train_test_split/no_normalization")
data_gene_test<-read.csv("data_test_overall.csv", h = T)
datExpr_test<-data_gene_test[, -c(1:9)]

MEs_train = moduleEigengenes(datExpr, moduleColors)$eigengenes
data_train_eigengene<-cbind(data_gene[, c(1:9)], MEs_train)
...
data_test_eigengene<-cbind(data_gene_test[, c(1:9)], MEs_test)
write.csv(data_test_eigengene, file = "data_test_eigengene_overall.csv", row.names = F)

# divide and export data
export_matched_data<-function(root_path, file_names, data_train_gene){
...
file_names_export<-paste("tune", seq(1:5), sep = "_")
root_dir<-"D:/AUTOSurv/ICGC_data/Caldas2007/divided_data_new/div_5"

export_matched_data(root_dir, file_names_export, data_train_eigengene)
